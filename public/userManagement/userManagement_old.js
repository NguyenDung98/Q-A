...
function saveVal(){
    ...
    if(editing == true){
        ...
        if(cellCurent.closest('table').id == 'lecturer-list'){
            lecturers[cellCurent.closest('tr').rowIndex].user = val;
            userId = lecturers[cellCurent.closest('tr').rowIndex].id
        }
        else {
            students[cellCurent.closest('tr').rowIndex].user = val;
    ...
    .catch(error => {
        ...
        cellCurent.innerHTML = "<tr>"+
            "<div class=\"session\">"+
            "<div class=\"session-info\">"+
            `<div class=\"session-name\"><div class=\"name\">${val}</div>`+
            `</div>`+
            "</div>"+
            "</div>"+
            "</tr><br>";
