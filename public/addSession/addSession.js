...
function deleteEvent(obj){
	...
	if (r) {obj.parentNode.parentNode.parentNode.parentNode.parentNode.removeChild(obj.parentNode.parentNode.parentNode.parentNode);}
}
...
function addSession(newSession){
	...
	// 	// var bg = eventBegin.value;
		...
		newEvent.innerHTML = "<div class=\"w3-card w3-white w3-round-medium\" style=\"margin: 15px 5px 5px 5px; cursor: pointer;\" onmouseover=\"mOver(this)\" onmouseout=\"mOut(this)\">" +
							 ...
							 "	  <div class=\"w3-bar\">" + 
							 `		  <label class=\"w3-bar-item w3-large w3-text-teal\" style=\"display: block;\">${newSession.password}</label>` + 
							 ...
							 `	  <label class=\"w3-small\" style=\"display: block; opacity: 0.5;\">${newSession.beginDate}+${newSession.endDate}</label>` +
							 "	  <br><br>" +
							 "	  <a href='/' target=_blank type=\"button\" class=\"w3-button w3-block hide w3-teal w3-hover-teal\" style=\"opacity:1.0\">Load</a>" +
							 "  </div>" +
							 "</div>";
...
function getVal(){
    ...
    let code = eventPassword.value;
    let bdate = eventBegin.value;
    let edate = eventEnd.value;
    if (nameEvent==""){
		alert("Hãy thêm tên sự kiện");
	}
	else if(code==""){
